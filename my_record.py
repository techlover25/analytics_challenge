...
# Another challenge was while reading the results file, if the result for the last dataset is null, the program fail with an error. But this is handled now.
# While sorting the datasets and algorithms, I was struggling how it could be implemented. But lambda function made it really easy and I realized how learning it is beneficial.
...
class Algorithm:
    ...
    def get_score(self, results):
        ...
        # Below logic for getting the result for each dataset_id into list of list : result_data 
        ...
        # Creating a dictionary to store the results for each dataset_id
        datasets_result = {}

        for data in result_data:
            dataset_id, result = data

            if dataset_id in datasets_result:
                datasets_result[dataset_id].append(result)
            else:
                datasets_result[dataset_id] = [result]

        # Identifying the top 3 results for each datasets and fetching its corresponding indices
        dataset_indices = []

        for results in datasets_result.values():#[5]
            sorted_results = sorted(enumerate(results, 0), key=lambda x: x[1], reverse=True)
            result_indices = []
    
            # Getting the indices for top 1, 2, and 3
            for i, rank in enumerate(sorted_results):
                result_indices.append(int(rank[0]))
                if i >= 2:
                    break
            dataset_indices.append(result_indices)
    
        # Assigning the algorithm score for the top 3 indices
        score_list = []

        for rank in dataset_indices:
        ...
        # Calculating the score for each algorithm
    ...
    def compute(self, results, average_result, count, nfail, ongoing, faildataset):
        ...
        for result in results:
            if result.get_algorithm_name() == self.name:
                for i in result.get_results():
                    if i[1] == '':
                        if i[0].endswith('S'):
...
# References
#[1] “Here is how to check if a string is a float in Python,” pythonhow.com. https://pythonhow.com/how/check-if-a-string-is-a-float/ (accessed Oct. 26, 2023).
#[2] “Here is how to check if a text file is empty in Python,” pythonhow.com. https://pythonhow.com/how/check-if-a-text-file-is-empty/ (accessed Oct. 27, 2023).
#[3] Aimen Jabeen, “How to find the max value in a dictionary in python?,” Entechin, (September 24, 2023).Accessed October 28, 2023.[Blog].Available: https://www.entechin.com/how-to-find-the-max-value-in-a-dictionary-in-python/.
#[4] Drazen Zaric, “Python sort lambda - How to sort with lambda key function in Python,” Blogboard Journal, (Jun. 14, 2022).Accessed October 28, 2023.[Blog].Available: https://blogboard.io/blog/knowledge/python-sorted-lambda/.
#[5] GeeksforGeeks. “Python - Index Ranks of Elements,” GeeksforGeeks, Mar. 25, 2021. https://www.geeksforgeeks.org/python-index-ranks-of-elements/ (accessed Oct. 28, 2023).
#[6] GeeksforGeeks. “How to use strptime with milliseconds in Python,” GeeksforGeeks, Aug. 23, 2021. https://www.geeksforgeeks.org/how-to-use-strptime-with-milliseconds-in-python/ (accessed Oct. 29, 2023).